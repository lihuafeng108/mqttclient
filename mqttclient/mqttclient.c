...
MQTT_CLIENT_SET_DEFINE(userdata, void*, NULL)
...
//----------------------------- add by lihf ---------------------------------------------------------
int mqtt_is_connect_success(mqtt_client_t *c)
{
	if(c)
	{
		return mqtt_is_connected(c);
	}
	return MQTT_NULL_VALUE_ERROR;
}


const char* mqtt_errormsg(int error_no)
{
	char *msg = NULL;
	switch(error_no)
	{
	case MQTT_SSL_CERT_ERROR: msg = "cetr parse failed"; break;
    case MQTT_SOCKET_FAILED_ERROR: msg = "socket fd failed";break;
    case MQTT_SOCKET_UNKNOWN_HOST_ERROR: msg = "socket unknown host ip or domain";break;
    case MQTT_SET_PUBLISH_DUP_FAILED_ERROR: msg = "mqtt publish packet set udp bit failed";break;
    case MQTT_CLEAN_SESSION_ERROR: msg = "mqtt clean session error";break;
    case MQTT_ACK_NODE_IS_EXIST_ERROR: msg = "mqtt ack list is exist ack node";break;
    case MQTT_ACK_HANDLER_NUM_TOO_MUCH_ERROR: msg = "mqtt ack handler number is too much";break;
    case MQTT_RESUBSCRIBE_ERROR: msg = "mqtt resubscribe error";break;
    case MQTT_SUBSCRIBE_ERROR: msg = "mqtt subscribe error";break;
    case MQTT_SEND_PACKET_ERROR: msg = "mqtt send a packet";break;
    case MQTT_SERIALIZE_PUBLISH_ACK_PACKET_ERROR: msg = "mqtt serialize publish ack packet error";break;
    case MQTT_PUBLISH_PACKET_ERROR: msg = "mqtt publish packet error";break;
    case MQTT_RECONNECT_TIMEOUT_ERROR: msg = "mqtt try reconnect, but timeout";break;
    case MQTT_SUBSCRIBE_NOT_ACK_ERROR: msg = "mqtt subscribe, but not ack";break;
    case MQTT_NOT_CONNECT_ERROR: msg = "mqtt not connect";break;
    case MQTT_SUBSCRIBE_ACK_PACKET_ERROR: msg = "mqtt subscribe, but ack packet error";break;
    case MQTT_UNSUBSCRIBE_ACK_PACKET_ERROR: msg = "mqtt unsubscribe, but ack packet error";break;
    case MQTT_PUBLISH_ACK_PACKET_ERROR: msg = "qtt pubilsh ack packet error";break;
    case MQTT_PUBLISH_ACK_TYPE_ERROR: msg = "mqtt pubilsh ack type error";break;
    case MQTT_PUBREC_PACKET_ERROR: msg = "mqtt pubrec packet error";break;
    case MQTT_BUFFER_TOO_SHORT_ERROR: msg = "mqtt buffer too short";break;
    case MQTT_NOTHING_TO_READ_ERROR: msg = "mqtt nothing to read";break;
    case MQTT_SUBSCRIBE_QOS_ERROR: msg = "mqtt subsrcibe qos error";break;
    case MQTT_BUFFER_OVERFLOW_ERROR: msg = "mqtt buffer overflow";break;
    case MQTT_CONNECT_FAILED_ERROR: msg = "mqtt connect failed";break;
    case MQTT_MEM_NOT_ENOUGH_ERROR: msg = "mqtt memory not enough";break;
    case MQTT_NULL_VALUE_ERROR : msg = "mqtt value is null";break;
    case MQTT_FAILED_ERROR: msg = "failed";break;
    case MQTT_SUCCESS_ERROR: msg = "success";break;
	default:msg="unknown";break;
	}
	return msg;
}

void* mqtt_get_userdata(mqtt_client_t *c)
{
	if(c)
	{
		return c->mqtt_userdata;
	}
	return NULL;
}
...
//--------------------------- add log func ----------------------------------->
#if 0
static PLOG_FUNCTION gs_plogfunc=NULL;
int log_func(const char *tag, int type, int log, const char *format, ...)
{
     char buffer[1024] = {0};
    va_list argp;
    va_start(argp, format);
    vsnprintf(buffer, 1020, format, argp);
    va_end(argp);

    if(gs_plogfunc)
    {
        if(type == MQTT_LOG_INFO_LEVEL) type = 1 ;
        else if(type == MQTT_LOG_WARN_LEVEL) type = 2 ;
        else if(type == MQTT_LOG_ERR_LEVEL) type = 3 ;
        else type = 0;
        gs_plogfunc(tag, type, log, "%s", buffer);
    }
    else if(log)
    {
        printf("/%s %s\n", tag, buffer);
...
void mqtt_set_log(PLOG_FUNCTION logfunc)
{
    gs_plogfunc = logfunc;
}
#endif
